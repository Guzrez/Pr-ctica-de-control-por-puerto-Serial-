...
#include <Arduino.h>
#include <DHT.h>
#include <ESP32Servo.h>  // Importamos la librería ESP32Servo
...
Servo servo1;                   // Declaramos el servomotor
...
void setup() {
  ...
  analogWrite(ventilador, 0);   // Apagamos ventilador
  analogWrite(foco, 0);         // Apagagamos foco
  digitalWrite(BOMBAPIN, LOW);  // Bomba se inicializa apagada
  
  // Configuración de servomotor
  servo1.attach(SERVOPIN1);     // Conectamos el servomotor al pin definido
...
void loop() {
  ...
  if (tiempoActual - ultimaActualizacion >= intervaloActualizacion) {
    ...
    // Leemos y calculamos la temperatura promedio de los sensores LM35
    ...
    // Controlamos el foco y ventilador solo si se ha recibido una temperatura
    ...
    // Apagamos los dispositivos manualmente
  ...
  // Procesamos los comandos de la GUI
  if (Serial.available()) {
